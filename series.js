 ...
 * 还可以以json的形式来提供tasks。每一个属性都会被当作函数来执行，并且结果也会以json形式传给series最后的那个callback。这种方式可读性更高一些。
 */
...
// 1.1
async.series([
    function(cb) { var n = 2; cb(null, n); },
    function(cb) { var n = 3; cb(null, n*n); },
    function(cb) { var n = 6; cb(null, n*n); }
], function(err, results) {//results是所有函数执行结果组成的数组
    log('err: ', err);
    log('results: ', results);
});
//err: null
